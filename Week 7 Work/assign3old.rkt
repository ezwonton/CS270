...
; an arbitrary boolean expression into an equivalent one in conjunctive
...
;
; For this assignment we assume that boolean expression's only consist of
...
;  cnf? can be used to help test cnf.
;  
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(define (no-or-above-and? expr)
  (cond
    ...
    [else (disjunction? expr) (and (no-and? (op1 expr)) (no-and? (op2 expr)))]
  )
)
...
(define (no-and? expr)
  (cond
    [(variable? expr) #t]
    [(negation? expr) #t]
    [(disjunction? expr) (no-and? (first (rest expr)))]
    [else (conjunction? expr) #f]
  )
)
#|
(define expr1 '(or (or a b) c))
(define expr2 '(or (and a b) c))
(define expr3 '(and (or a b) c))

(define expr4'(or a (or b c)))
(define expr5'(or a (and b c)))
(define expr6'(and a (or b c)))

(no-or-above-and? expr1)
(no-or-above-and? expr2)
(no-or-above-and? expr3)

(no-or-above-and? expr4)
(no-or-above-and? expr5)
(no-or-above-and? expr6)
...
(define (nnf2cnf expr)
  (cond
    ...
    [else (disjunction? expr) (if (no-or-above-and? expr)
                                  expr
                                  (distrib-orand (op1 expr) (op2 expr)))]
  )  
)
...
(define (distrib-orand expr1 expr2)
  (cond
    [(conjunction? expr1) (list 'and (distrib-orand (op1 expr1) expr2) (distrib-orand (op2 expr1) expr2))]
    [(conjunction? expr2) (list 'and (distrib-orand expr1 (op1 expr2)) (distrib-orand expr1 (op2 expr2)))]
...
(distrib-orand 'u '(and x y))
(distrib-orand '(and u w) '(and x y))
|#
...
the function is-tautology? is provided in Part 0.

|#

(define (is-equivalent? expr1 expr2)
  (equal? (is-tautology? expr1) (is-tautology? expr2))
)
...
(check-equal? 
  (is-equivalent? (cnf '(or (and x (and y z)) w))
                     '(and (or x w) (and (or y w) (or z w)))) #t)
)
