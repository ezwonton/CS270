...
the builtin Boolean functions and must only use if expressions and
possibley the boolean constants with #t, #f.  This exercise shows that
all Boolean functions can be implemented using if and hence they need
not be primitive.
...
they have passed all of their associated unit tests, you are to write
...
In the second part of the assignment, students are to implement
...
(define (andi e1 e2)
  (cond
    [(null? e1) (display "no value for e1")]
    [(null? e1) (display "no value for e2")]
    [(equal? e1 e2) e1]
    [else #f]
  )
)
...
(define (ori e1 e2)
  (cond
    [(null? e1) (display "no value for e1")]
    [(null? e1) (display "no value for e2")]
    [(equal? (equal? (equal? e1 #f) (equal? e2 #f)) #t) e1]
    [else #t]
  )
)
...
(define (xori e1 e2)
  (cond
    [(null? e1) (display "no value for e1")]
    [(null? e1) (display "no value for e2")]
    [(equal? (equal? (equal? e1 #f) (equal? e2 #f)) #t) #f]
    [else #t]
  )
)
...
(define (impliesi e1 e2)
  (cond
    [(null? e1) (display "no value for e1")]
    [(null? e1) (display "no value for e2")]
    [(equal? (equal? (equal? e1 #t) (equal? e2 #f)) #t) e2]
    [else #t]
  )
)
...
(define (iffi e1 e2)
  (cond
    [(null? e1) (display "no value for e1")]
    [(null? e1) (display "no value for e2")]
    [(equal? (equal? (equal? e1 #f) (equal? e2 #f)) #t) #t]
    [else #f]
  )
)
...
(define-test-suite testimpliesequiv
  (check-equal?
    (impliesi #f #f) (ori (noti #f) #f)
  )
  (check-equal?
    (impliesi #f #t) (ori (noti #f) #t)
  )
  (check-equal?
    (impliesi #t #f) (ori (noti #t) #f)
  )
  (check-equal?
    (impliesi #t #t) (ori (noti #t) #t)
  )
)
