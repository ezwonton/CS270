...
Assignment 1 Solution (Please do NOT distribute)
...
There are a total of 13 questions.  All but one of the questions asks
you to implement the specified function and to test it with the provided
unit tests, which serve as further specification.  The one question
that does not ask for a function definition asks you to provide a
...
(define-test-suite testiffequiv
  (check-equal?
    (iffi #f #f) (and (implies #f #f) (implies #f #f)))
  (check-equal?
    (iffi #f #t) (and (implies #f #t) (implies #t #f)))
  (check-equal?
    (iffi #t #f) (and (implies #t #f) (implies #f #t)))
  (check-equal?
    (iffi #t #t) (and (implies #t #t) (implies #t #t)))
)
...
; all possible values of e1 and e2.


; (implies e1 e2) <-> (ori (noti e1) e2)
(define-test-suite testimpliesequiv
  (check-equal?
    (impliesi #f #f) (or (not #f) #f))
  (check-equal?
    (impliesi #f #t) (or (not #f) #t))
  (check-equal?
    (impliesi #t #f) (or (not #t) #f))
  (check-equal?
    (impliesi #t #t) (or (not #t) #t))
)
...
(define (atleastone L)
  (if (null? L)
      #f
      (or (= (first L) 1) (atleastone (rest L)))))
...
(define (exactlyone L)
  (if (null? L)
      #f
      (or (and (exactlyone (rest L)) (not (= (first L) 1)))
          (and (not (atleastone (rest L))) (= (first L) 1)))))
...
(define (oddones L)
  (if (null? L)
      #f
      (or (and (oddones (rest L)) (not (= (first L) 1)))
          (and (not (oddones (rest L))) (= (first L) 1)))))
