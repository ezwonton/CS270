...
; Why are quotes needed before x and (x a b c)?
...
Use an or expression to combine the last two cases.
Create and run a unit test similar to member-suite that tests alt-member?
Why is it equivalent to the version above?  
|#
(define alt-member?
  (lambda (a lat)
    (cond
      [(null? lat) #f]
      [else (or (eq? (first lat) a) (alt-member? a (rest lat)))])))

; Unit tests - tests alt-member?.
