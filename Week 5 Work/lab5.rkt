...
(define (boolean-expr? expr)
  (cond
    ...
    [(implication? expr) (and (boolean-expr? (op1 expr))
                              (boolean-expr? (op2 expr)))]
    [else #f]))
...
(define (get-variables expr)
  (cond
    [(constant? expr) '()]
    [(variable? expr) (union '() (list expr))]
    [(negation? expr) (union '() (list (second expr)))]
    [else (union (get-variables (op1 expr)) (get-variables (op2 expr)))]
  )
...
|#
(define (tt_helper case other)
  (map (lambda (x) (cons case x)) other)
)

(tt_helper '(a #t) '( ((b #f) (c #t)) ((b #t) (c #f)) ) )
'( ((a #t) (b #f) (c #t)) ((a #t) (b #t) (c #f)) )

(define (generate-truth-table vars)
  (cond
    [(null? vars) '(())]
    ;[(variable? (get-var-in-binding vars))
    ; (list (list (cons (get-var-in-binding vars) '(#t))) (list (cons (get-var-in-binding vars) '(#f))))]
    ;[else (list (cons (get-var-in-binding vars) '(#t)) (get-var-in-binding (rest vars))
    ;            (cons (get-var-in-binding vars) '(#f)) (get-var-in-binding (rest vars)))]
    
    [else (append (tt_helper (list (first vars) #t) (generate-truth-table (rest vars)))
               (tt_helper (list (first vars) #f) (generate-truth-table (rest vars))))]
  )
...
(define (taut_check expr TT)
  (map (lambda (x) (boolean-eval expr x)) TT)
)

(define (is-tautology? expr)
  (foldr (lambda (x boo) (and boo x)) #t (taut_check expr (generate-truth-table (get-variables expr)))
  ;(and (boolean-eval expr (first (generate-truth-table (get-variables expr)))
  ;     (boolean-eval expr (rest (generate-truth-table (get-variables expr)))
  ;(cond
    ;[(constant? expr) null]
    ;[(variable? expr) (cons expr '())]
    ;[(negation? expr) (get-variables (op1 expr))]
    ;[else (union (get-variables (op1 expr)) (get-variables (op2 expr)))]
    ;[(boolean-expr? expr)(boolean-eval expr)]
  )
) 
